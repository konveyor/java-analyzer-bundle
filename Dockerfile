FROM registry.access.redhat.com/ubi10/ubi AS jdtls-download
WORKDIR /jdtls
...
FROM registry.access.redhat.com/ubi10/ubi AS addon-build
...
RUN /usr/local/apache-maven-3.9.15/bin/mvn clean install -DskipTests=true
...
ENV M2_HOME /usr/local/apache-maven-3.9.15
...
COPY --from=addon-build /usr/local/apache-maven-3.9.15/ /usr/local/apache-maven-3.9.15/
RUN ln -s /usr/local/apache-maven-3.9.15/bin/mvn /usr/bin/mvn
