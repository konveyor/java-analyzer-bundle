...
RUN curl -fsSL -o /tmp/apache-maven.tar.gz https://dlcdn.apache.org/maven/maven-3/3.9.16/binaries/apache-maven-3.9.16-bin.tar.gz && \
    tar -xzf /tmp/apache-maven.tar.gz -C /usr/local/ && \
    rm /tmp/apache-maven.tar.gz
WORKDIR /app
COPY ./ /app/
ENV JAVA_HOME /usr/lib/jvm/java-21-openjdk
RUN /usr/local/apache-maven-3.9.16/bin/mvn clean install -DskipTests=true
...
ENV M2_HOME /usr/local/apache-maven-3.9.16
...
COPY --from=addon-build /usr/local/apache-maven-3.9.16/ /usr/local/apache-maven-3.9.16/
RUN ln -s /usr/local/apache-maven-3.9.16/bin/mvn /usr/bin/mvn
